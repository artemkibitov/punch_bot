...
registerAction('manager:object:employee:onboard:confirm', async (ctx, payload) => {
  ...
  try {
    // Используем use case для назначения сотрудника на объект
    const assignEmployeeToObjectUseCase = await container.getAsync('AssignEmployeeToObjectUseCase');
    const assignment = await assignEmployeeToObjectUseCase.execute(
...
registerAction('employee:unassign', async (ctx, payload) => {
  ...
  try {
    // Используем use case для снятия сотрудника с объекта
    const unassignEmployeeFromObjectUseCase = await container.getAsync('UnassignEmployeeFromObjectUseCase');
    await unassignEmployeeFromObjectUseCase.execute(
      parseInt(employeeId, 10),
      parseInt(objectId, 10),
      currentUser.id
    );
...
registerAction('object:shift:confirm:start', async (ctx, payload) => {
  ...
  try {
    // Используем use case для подтверждения начала смены
    const confirmShiftStartUseCase = await container.getAsync('ConfirmShiftStartUseCase');
    const { shift, workLogs } = await confirmShiftStartUseCase.execute(
      parseInt(shiftId, 10),
      manager.id,
      null,
      [] // нет отсутствующих
...
registerAction('shift:add:employee:confirm', async (ctx, payload) => {
  ...
  try {
    const addEmployeeToShiftUseCase = await container.getAsync('AddEmployeeToShiftUseCase');
    const workLog = await addEmployeeToShiftUseCase.execute(
      parseInt(shiftId, 10),
      parseInt(employeeId, 10),
      manager.id
    );
...
registerAction('shift:remove:employee', async (ctx, payload) => {
  ...
  try {
    const removeEmployeeFromShiftUseCase = await container.getAsync('RemoveEmployeeFromShiftUseCase');
    const workLog = await removeEmployeeFromShiftUseCase.execute(
...
registerAction('object:shift:confirm:end', async (ctx, payload) => {
  ...
  try {
    // Используем use case для подтверждения окончания смены
    const confirmShiftEndUseCase = await container.getAsync('ConfirmShiftEndUseCase');
    const { shift, workLogs } = await confirmShiftEndUseCase.execute(
      parseInt(shiftId, 10),
      manager.id
    );
...
registerAction('object:shift:create', async (ctx, objectId) => {
  ...
  try {
    // Используем use case для создания смены
    const createShiftUseCase = await container.getAsync('CreateShiftUseCase');
    const today = new Date().toISOString().split('T')[0];
    const shift = await createShiftUseCase.execute(parseInt(objectId, 10), today, { isAdmin: manager.role === 'ADMIN' });
...
registerAction('object:edit:status:confirm', async (ctx, payload) => {
  ...
  try {
    // Используем use case для обновления объекта
    const updateObjectUseCase = await container.getAsync('UpdateObjectUseCase');
    const updatedObject = await updateObjectUseCase.execute(
      parseInt(objectId, 10),
      { status: newStatus },
      {
        managerId: manager.id,
